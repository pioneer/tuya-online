...
def test_tuya_connection():
    ...
    try:
        client = TuyaClient(config.tuya.endpoint, config.tuya.access_id, config.tuya.access_key)
...
def run_single_poll():
    ...
    try:
        ...
        print(
            f"  📝 New state: observed={new_state.last_observed_online}, streak={new_state.streak}"
        )
